...
%token <int_val>   CARROTS
%token <int_val>   CARROT
%token <int_val>   PLUS
%token <int_val>   MINUS
%token <int_val>   MULT
%token <int_val>   DIV
...
expr:       expr EQUALS addit   { $$ = new cBinaryExprNode($1, $2, $3); }
        |   expr CARROTS addit  { $$ = new cBinaryExprNode($1, $2, $3); }
        |   addit               { $$ = $1; }

addit:      term CARROT addit   { $$ = new cBinaryExprNode($1, $2, $3); }
        |   addit PLUS term     { $$ = new cBinaryExprNode($1, $2, $3); }
        |   addit MINUS term    { $$ = new cBinaryExprNode($1, $2, $3); }
        |   term                { $$ = $1; }

term:       term MULT fact      { $$ = new cBinaryExprNode($1, $2, $3); }
        |   term DIV fact       { $$ = new cBinaryExprNode($1, $2, $3); }
        |   term MOD fact       { $$ = new cBinaryExprNode($1, $2, $3); }
        |   fact                { $$ = $1; }
